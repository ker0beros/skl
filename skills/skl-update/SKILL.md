---
name: skl-update
description: "Update this project's skl install to the latest version on GitHub. Pulls the latest from the main branch of ker0beros/skl and syncs all the skl-* skills + the shared agents into this project's .claude/, PRESERVING each resources/project.config.md (your surface, gate commands, gate_strictness). Does not run the installer script. Reload Claude Code afterward so the refreshed skills register."
...
2. **Show what's new.** If `OLD` is set and `OLD != NEW`, print `git -C ~/.skl log --oneline
