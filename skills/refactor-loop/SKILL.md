...
This command does **not** use `speckit-all`; it chains the individual `speckit-*` skills directly via the Skill tool. It is a sibling of `/go-loop` and shares the same QA agents.
...
4. From the analysis, write `.refactor-loop/backlog.md` — a **prioritized** list (highest severity first). Each item: `id · title · why · files · current → target state · risk · severity (Critical/High/Medium/Low) · est. effort`. Use `resources/refactor-backlog-template.md`. No human gate — proceed straight to Phase 2.

## Phase 2 — Autonomous refactor loop

5. **Set up the review branch.** Create an integration branch `refactor-loop/<short-stamp>` off the current branch and stay on it. All passing items merge here; this branch is what the user reviews at the end. **Never push, and never merge into the default / main / dev branch.**

Then, for each backlog item, highest-severity first, with `iter = 1`:

6. **Scaffold (the audit is the spec → start at plan).** Invoke `speckit-specify` (Skill tool) **non-interactively**, seeded with the item description + the `.refactor-loop/organization.md` target state. It creates branch `NNN-refactor-<slug>` + `specs/NNN-refactor-<slug>/spec.md` off the integration tip. **Skip `speckit-clarify`** — the audit item is the spec. Record `Refactor-Item: <id>` in the spec.
7. **Build.** `speckit-plan → speckit-tasks → speckit-analyze → speckit-checklist → speckit-implement` (Skill tool). On `iter > 1`, seed `speckit-plan` with the previous remediation brief (`specs/<feature>/.refactor-loop/remediation-iter-<iter-1>.md`).
8. **Gate.** Run the automated gates from `project.config.md` (the **behavior-preservation net** — e.g. `make analyze`, `make test`); if red, this round failed. Then spawn the **6 QA agents** (Agent tool, parallel) seeded per `resources/refactor-pass-matrix.md`. A gate passes with **0 Critical / 0 High / 0 Medium**.
9. **Resolve the item.**
   - **All pass** → `git add -A && git commit -m "refactor(NNN): <slug>"` on the item branch; `git checkout <integration> && git merge --no-ff <item-branch>`; fire `bash ~/.claude/notify-telegram.sh "[<project>] /refactor-loop ✅ <id> <slug>"`; mark **done**; next item.
   - **Fail & `iter < 10`** → spawn `ultrathink-debugger` with the aggregated findings + failing gate output; it writes `specs/<feature>/.refactor-loop/remediation-iter-<iter>.md` (root cause + ordered, file-keyed fix list) using `resources/remediation-brief-template.md`; `iter += 1`; **go to step 7**.
   - **Fail & `iter == 10`** → mark the item **DEFERRED** (log why); `git checkout <integration>` and leave the item branch unmerged; fire `bash ~/.claude/notify-telegram.sh "[<project>] /refactor-loop ⚠️ DEFERRED <id> after 10 tries"`; next item.
...
10. When the backlog is drained, **re-audit** the integration tip (repeat Phase 0/1 analysis, scope-limited). 
    ...
    - If it surfaces **no Critical/High** (only Low/nice-to-haves) **and** the latest automated gates + QA panel are green → write `.refactor-loop/report.md` (items done / deferred / per-iteration history / remaining Low items), fire `bash ~/.claude/notify-telegram.sh "[<project>] /refactor-loop done — <N> refactored, <M> deferred"`, present the summary + the integration branch name, and **STOP**.
...
- **Working files**: `.refactor-loop/` at the repo root (organization, backlog, report); per-item remediation under `specs/<feature>/.refactor-loop/`.
