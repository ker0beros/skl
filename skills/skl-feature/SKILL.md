...
13. **Evaluate the pass matrix.** Read `gate_strictness` from `resources/project.config.md` (missing ⇒ `standard`). Severity ladder: Critical › High › Medium › Low › Info. A gate **passes** iff it reports **0 Critical, 0 High, and 0 Medium** — plus, depending on mode: `standard` also needs **0 Low** (Info logged); `strict` also needs **0 Low and 0 Info** (and the analyze gate must be clean of info-level diagnostics — fatal-on-info — not merely exit 0); `low` needs neither (Low + Info logged). The whole round **passes** iff: the automated gates were green (per the mode) **and** all 6 QA gates pass **and** *(design mode only)* every Visual Target + Animation Inventory row is present **and** *(design-driven mobile only)* `specs/<feature>/verification/` holds a live-render screenshot per design-driven screen — a green automated gate plus agent verdicts is **not** sufficient: without the render the round cannot pass (the render gate in `resources/pass-matrix.md`).
    ...
    - **Any fail and `iteration < 10`** → spawn `ultrathink-debugger` (Agent tool) with the **aggregated findings + failing gate output**; have it write a remediation brief (root cause + ordered, file/spec-keyed fix list) to `specs/<feature>/.skl-feature/remediation-iter-<iteration>.md` using `.claude/skills/skl-feature/resources/remediation-brief-template.md`. Fire `bash ~/.claude/notify-telegram.sh "[<project>] /skl-feature <feature> iter <iteration>/10 FAIL — <failing gates>; re-planning"`. Then `iteration = iteration + 1` and **go to step 10**.
