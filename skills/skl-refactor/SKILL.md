---
name: skl-refactor
description: "Autonomously refactor a codebase toward its declared architecture. Maps the code organization from the constitution (researching best practices and stopping for your sign-off to seed it if the constitution is silent), uses a skl-refactoring-specialist agent to audit deviations into a prioritized backlog, then refactors each item through the speckit workflow (plan→tasks→analyze→checklist) with the skl-refactoring-specialist carrying out the behavior-preserving transformation, QA-gates it, commits on a review branch, and loops — re-auditing until no Critical/High refactors remain and all gates are green."
...
compatibility: "Requires the .specify/ spec-kit structure, the speckit-* skills, and the agents in .claude/agents/ (the QA panel — skl-spec-auditor, skl-guideline-auditor, skl-pragmatist, skl-reality-checker, skl-code-reviewer, skl-security-auditor, skl-test-integrity-auditor, skl-ui-tester — plus skl-debugger and the skl-refactoring-specialist, which audits the code and performs the behavior-preserving refactors). Surface + gate commands come from resources/project.config.md (generated by /skl-init)."
metadata:
  author: "khairul"
  version: "1.0.0"
  source: "skills/skl-refactor"
user-invocable: true
...
`/skl-refactor` drives an **autonomous** refactor of the codebase toward its own declared architecture, and keeps going until the code is clean enough:
...
This command does **not** use `speckit-all`; it chains the individual `speckit-*` skills directly via the Skill tool. It is a sibling of `/skl-feature` and shares the same QA agents.
...
3. **Establish the target from the constitution.** The **code organization must be declared in `.specify/memory/constitution.md`** — that is the source of truth this refactor drives toward. Two paths:
   ...
   - **Constitution is silent on it** → **WebSearch** best-practice code organization for this project's stack/domain (cite the sources), synthesize a recommended organization (topology + conventions + LOC budgets, reconciled with the dominant existing patterns). Then:
     ...
     - **`--auto`** → **do not stop** — adopt the **highest-confidence** recommendation from the WebSearch (the one you judge best for this stack/domain), write it into the constitution, and clearly note what you chose (in your output + `.skl-refactor/report.md`) so the user can review it later.
     Either way, write the organization **into `.specify/memory/constitution.md`** as a *Code Organization* principle, then derive `.skl-refactor/organization.md` from the now-updated constitution exactly as above. Do not proceed to Phase 1 until the constitution declares it.

## Phase 1 — Audit (the refactor backlog)

4. From the analysis (the **`skl-refactoring-specialist`'s ranked smells** + the topology map), write `.skl-refactor/backlog.md` — a **prioritized** list (highest severity first), each item mapping a smell to its `organization.md` target: `id · title · why · files · current → target state · risk · severity (Critical/High/Medium/Low) · est. effort`. Use `resources/refactor-backlog-template.md`. No human gate — proceed straight to Phase 2.

## Phase 2 — Autonomous refactor loop

5. **Set up the review branch.** Create an integration branch `skl-refactor/<short-stamp>` off the current branch and stay on it. All passing items merge here; this branch is what the user reviews at the end. **Never push, and never merge into the default / main / dev branch.**

Then, for each backlog item, highest-severity first, with `iter = 1`:

6. **Scaffold (the audit is the spec → start at plan).** Invoke `speckit-specify` (Skill tool) **non-interactively**, seeded with the item description + the `.skl-refactor/organization.md` target state. It creates branch `NNN-refactor-<slug>` + `specs/NNN-refactor-<slug>/spec.md` off the integration tip. **Skip `speckit-clarify`** — the audit item is the spec. Record `Refactor-Item: <id>` in the spec.
7. **Build.** Plan the item with the speckit chain — `speckit-plan → speckit-tasks → speckit-analyze → speckit-checklist` (Skill tool). Then the **`skl-refactoring-specialist`** agent (Agent tool) **carries out the refactor**: apply the planned tasks in **small, incremental, behavior-preserving** steps (run the gates/tests after each step; smallest viable change; update all call-sites; never weaken tests), moving the code to the `.skl-refactor/organization.md` target. On `iter > 1`, seed both `speckit-plan` and the agent with the previous remediation brief (`specs/<feature>/.skl-refactor/remediation-iter-<iter-1>.md`). See `agents/skl-refactoring-specialist.md`.
8. **Gate.** Run the automated gates from `project.config.md` (the **behavior-preservation net** — e.g. `make analyze`, `make test`); if red, this round failed. Then spawn the **8 QA agents** (Agent tool, parallel) seeded per `resources/refactor-pass-matrix.md`. A gate passes with **0 Critical / 0 High / 0 Medium**, plus — by `gate_strictness` in `project.config.md` (default `standard`; toggle with `/skl-gate`) — **0 Low** in `standard`, **0 Low + 0 Info** in `strict` (run analyze fatal-on-info), or neither in `low` (Low + Info logged).
9. **Resolve the item.**
   - **All pass** → `git add -A && git commit -m "refactor(NNN): <slug>"` on the item branch; `git checkout <integration> && git merge --no-ff <item-branch>`; fire `bash ~/.claude/notify-telegram.sh "[<project>] /skl-refactor ✅ <id> <slug>"`; mark **done**; next item.
   - **Fail & `iter < 10`** → spawn `skl-debugger` with the aggregated findings + failing gate output; it writes `specs/<feature>/.skl-refactor/remediation-iter-<iter>.md` (root cause + ordered, file-keyed fix list) using `resources/remediation-brief-template.md`; `iter += 1`; **go to step 7**.
   - **Fail & `iter == 10`** → mark the item **DEFERRED** (log why); `git checkout <integration>` and leave the item branch unmerged; fire `bash ~/.claude/notify-telegram.sh "[<project>] /skl-refactor ⚠️ DEFERRED <id> after 10 tries"`; next item.
...
10. When the backlog is drained, **re-audit** the integration tip (repeat Phase 0/1 analysis, scope-limited). 
    ...
    - If it surfaces **no Critical/High** (only Low/nice-to-haves) **and** the latest automated gates + QA panel are green → write `.skl-refactor/report.md` (items done / deferred / per-iteration history / remaining Low items), fire `bash ~/.claude/notify-telegram.sh "[<project>] /skl-refactor done — <N> refactored, <M> deferred"`, present the summary + the integration branch name, and **STOP**.
...
- **Behavior preservation is cardinal.** Structure changes; observable behavior must not. Automated gates (analyze + tests) staying green is the proof; `skl-reality-checker` independently confirms by running it. Tests may be updated for moved/renamed symbols, but assertions/expectations must not be weakened to make a refactor "pass."
...
- **Working files**: `.skl-refactor/` at the repo root (organization, backlog, report); per-item remediation under `specs/<feature>/.skl-refactor/`.
