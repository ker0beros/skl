...
`/init-loop` is the **one-time setup** you run before `/go-loop` or `/refactor-loop`. It gets the project to the point where those loops have everything they need: the Spec Kit scaffolding, the QA panel, and — crucially — a **constitution that declares the code organization** (which `/refactor-loop` Phase 0 requires as its source of truth). Three phases:

- **Phase 1 — Install prerequisites**: ensure `uv`/`uvx`, initialize **Spec Kit** (`.specify/` + the `speckit-*` skills), verify the spec-loop QA agents, optionally bootstrap Playwright for web.
...
> ⚠️ **Reload checkpoint.** Spec Kit installs the `speckit-*` skills into the session. If this run has to initialize Spec Kit, those skills are **not registered until you reload Claude Code** — so `/init-loop` STOPS after Phase 1 and asks you to reload, then re-run `/init-loop` (it resumes at Phase 2, skipping the install).
...
2. **Spec Kit.** If `.specify/` is **absent**, initialize it in the project:
   ...
   - **If you just created `.specify/` this run → STOP HERE.** Fire `bash ~/.claude/notify-telegram.sh "[<project>] /init-loop installed Spec Kit — reload Claude Code, then re-run /init-loop"`, tell the user to **reload Claude Code and re-run `/init-loop`**, and end the turn. The `speckit-*` skills are not invokable until the reload.
   - If `.specify/` already existed, the speckit-* skills are already registered — continue.
3. **QA agents (verify, don't install).** Confirm the spec-loop QA agents are present in `.claude/agents/` or `~/.claude/agents/`: `Jenny`, `karen`, `claude-md-compliance-checker`, `code-quality-pragmatist`, `task-completion-validator`, `ui-comprehensive-tester`, `ultrathink-debugger`. If any are missing, run the spec-loop installer — try `~/.spec-loop/install.sh "$PWD"` or `~/Documents/spec-loop/install.sh "$PWD"` (or wherever spec-loop is checked out); if you can't locate it, tell the user to run `install.sh` themselves. These agents are needed by the loops, not by init.
4. **Playwright (web only, optional).** If `resources/project.config.md` exists and its `surface_default` is `web`/`both` while `playwright: absent`, offer to bootstrap it (`install.sh <project> --with-playwright`). It is only needed for `/go-loop` web parity; skip for mobile/refactor-only projects.
...
6. **Confirm the stack** with `AskUserQuestion` — present what you detected and ask for anything that changes the recommendation (domain, team size, expected scale, web/mobile/both). Fire `bash ~/.claude/notify-telegram.sh "[<project>] /init-loop awaiting stack confirmation"` before asking.
...
10. **Verify + report.** Confirm `.specify/memory/constitution.md` exists and contains:
    - a **Code Organization** principle that states the topology + dependency direction concretely (this is exactly what `/refactor-loop` Phase 0 keys on — if it's vague, edit the file to make it explicit), **and**
    - the **code-quality**, **testing/TDD**, **UX-consistency**, and **performance** principles.
    Then fire `bash ~/.claude/notify-telegram.sh "[<project>] /init-loop done — constitution seeded (<stack>, <org>)"`, present a short summary (stack, chosen org, the principle headings, the constitution path), and point to next steps: `/go-loop <design> — <intent>` or `/refactor-loop [scope]`.
...
- **Reload after a fresh Spec Kit init** — the `speckit-*` skills don't register until a reload; init-loop checkpoints there rather than failing to invoke `speckit-constitution`.
- **The constitution MUST declare the code organization.** That is the contract `/refactor-loop` Phase 0 depends on — don't finish Phase 3 until the constitution states the topology + dependency direction concretely.
- **The user picks the organization.** init-loop *recommends* (web-search-backed, with citations) but the decision is the user's (`AskUserQuestion`).
- **Non-destructive.** Spec Kit init merges into the project; it never removes your code. init-loop never touches git history and never pushes.
