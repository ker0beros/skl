# Granting issue access for `/loop-fix` (GitHub / GitLab)

> Followed by `/loop-fix` Phase 0 when fetching a linked issue fails with an **auth / permission /
...
backlog row and the spec's `Fix-Issue` line for traceability. `/loop-fix` does **not** modify or close
