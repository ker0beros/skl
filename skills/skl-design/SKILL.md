...
`/skl-design` is the **upstream design-ideation** step: it turns a rough feature idea into a
...
5. **Auto-proceed** — chain into `/skl-plan` to create the numbered, ready-to-run spec.
...
generate the UI; the plan it writes is then run by `/skl-run` (or `/skl-feature`).
...
1. **Design system check.** Fire `bash ~/.claude/notify-telegram.sh "[<project>] /skl-design awaiting design-system answer"`, then `AskUserQuestion`: *"Is your design system already defined in Claude Design?"*
...
5. **Auto-proceed — write the plan.** Invoke **`/skl-plan`** (Skill tool), seeded with: the brainstormed feature flow (journey + screens + states + interactions), the selected platforms, and a pointer to the Claude Design prompt file. If a Claude Design project reference exists (the user provided one in Step 1, or created one from the prompt), pass it so `/skl-plan` runs **design-driven**; otherwise `/skl-plan` runs **text-only** with the rich brainstorm as the spec. Fire `bash ~/.claude/notify-telegram.sh "[<project>] /skl-design done — prompt written + plan NNN queued"`, then **tell the user, in plain language, what they now have and the next step**: "✅ Two things are ready: (1) a Claude Design prompt to generate the look, and (2) plan NNN, queued to build. Next: paste the prompt into claude.ai/design (if you haven't), then run `/skl-run` to build it."
...
- **Auto-chains to `/skl-plan`** — the deliverables are the Claude Design prompt **and** a numbered, ready-to-run plan. It does not build (that's `/skl-run` / `/skl-feature`).
...
- **Working files**: `.skl-design/` at the repo root (the design-system prompt + per-feature Claude Design prompt).
