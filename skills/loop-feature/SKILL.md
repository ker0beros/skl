...
- **No auto-commit.** `/loop-feature` always stops after the matrix (pass or cap) so a human can **view the app rendered on a device/simulator and compare it to the design reference** (`specs/<feature>/verification/` vs `references/`) before committing.
