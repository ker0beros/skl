...
4. From the analysis, write `.loop-refactor/backlog.md` — a **prioritized** list (highest severity first). Each item: `id · title · why · files · current → target state · risk · severity (Critical/High/Medium/Low) · est. effort`. Use `resources/refactor-backlog-template.md`. No human gate — proceed straight to Phase 2.
...
7. **Build.** `speckit-plan → speckit-tasks → speckit-analyze → speckit-checklist → speckit-implement` (Skill tool). On `iter > 1`, seed `speckit-plan` with the previous remediation brief (`specs/<feature>/.loop-refactor/remediation-iter-<iter-1>.md`).
