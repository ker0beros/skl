...
compatibility: "Requires the .specify/ spec-kit structure, the speckit-* skills, and the agents in .claude/agents/ (the QA panel — Jenny, karen, claude-md-compliance-checker, code-quality-pragmatist, task-completion-validator, ui-comprehensive-tester, ultrathink-debugger — plus the refactoring-specialist, which audits the code and performs the behavior-preserving refactors). Surface + gate commands come from resources/project.config.md (generated by /loop-init)."
metadata:
  author: "khairul"
  version: "1.0.0"
