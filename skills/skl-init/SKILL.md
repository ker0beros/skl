...
3. **Skills + agents present (verify).** Confirm the skl skills live in `.claude/skills/skl-*/` and the agents in `.claude/agents/` (or `~/.claude/agents/`): `Jenny`, `karen`, `claude-md-compliance-checker`, `code-quality-pragmatist`, `task-completion-validator`, `ui-comprehensive-tester`, `ultrathink-debugger`, `business-analyst`, `refactoring-specialist`. If any are missing, they're installed by **pulling the GitHub repo into `.claude/`** — tell the user to run the install one-liner from the README (clone/pull `ker0beros/skl` to `~/.skl`, then `rsync` `skills/` + `agents/` into `.claude/`) and **reload**, or run `/skl-update`. There is no install script.
