# Code-organization playbook — starting hypotheses (skl-init Phase 2)

> Used by `/skl-init` to seed its **recommendation**, NOT to replace the web search. Treat every
...
> naming + LOC budgets — because the QA `skl-guideline-auditor` gate reads the constitution as its target.
...
| Size budgets | LOC ceilings per file/class/function (so the guideline gate has a measure) |
