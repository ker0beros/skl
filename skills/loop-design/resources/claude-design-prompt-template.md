# Claude Design prompt — <feature> (loop-design Step 4 output)

> The shape of the prompt `/loop-design` writes to `.loop-design/<slug>/claude-design-prompt.md`.
...
- Animations worth specifying: *element · trigger · property · duration · easing · loop?* (these become
  the spec's Animation Inventory when `/loop-plan` runs design-driven).
...
> After generating in claude.ai/design, the design becomes the visual spec that `/loop-plan` /
> `/loop-feature` pull (read-only) and verify parity against.
