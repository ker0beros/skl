# Design-system checklist — step-by-step (loop-design Step 1, "No")

> Used by `/loop-design` when the user has **no** design system in Claude Design yet. Drive it with
> Superpowers `brainstorming` — settle each item **one at a time**, asking the user, then write the
> result as `.loop-design/design-system-prompt.md` (a prompt the user pastes into claude.ai/design to
...
Write `.loop-design/design-system-prompt.md` as a Claude Design prompt that asks for a **reusable design
