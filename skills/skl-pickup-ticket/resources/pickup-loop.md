...
> The exact per-provider commands for **polling** the `loop-ready` queue, **opening the PR/MR**,
> **relabelling + commenting** on defer, and the **wait** between polls. SKILL.md sequences these; this
> file is the source of truth for the commands. Reuse `skl-fix/resources/issue-access.md` if a fetch
> hits an auth wall. Never merge; never apply `loop-ready` yourself.
...
**Poll — oldest open `loop-ready` issue** (fetch a small batch, then drop skip-listed ids and take the
oldest remaining):
```bash
gh issue list \
  --search 'label:"loop-ready" state:open sort:created-asc' \
  --limit 10 \
  --json number,title,body,url,labels,createdAt
```
The first element (after removing skip-listed `number`s) is the ticket. Empty array → no eligible ticket.
...
```bash
gh issue edit <n> --remove-label "loop-ready" --add-label "loop-blocked"
...
**Poll — oldest open `loop-ready` issue.** The API is the most reliable JSON source:
```bash
[GITLAB_HOST=<host>] glab api \
  "projects/:id/issues?labels=loop-ready&state=opened&order_by=created_at&sort=asc&per_page=10"
```
(`glab` resolves `:id` to the current project.) Drop skip-listed `iid`s; take the oldest remaining. The
issue's per-project number is `iid`. (Equivalent CLI: `glab issue list --label loop-ready --sort asc`.)
...
**On defer — relabel + comment:**
```bash
[GITLAB_HOST=<host>] glab issue update <iid> --unlabel "loop-ready" --label "loop-blocked"
