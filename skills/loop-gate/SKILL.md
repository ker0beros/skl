...
`/loop-gate` is the QA-gate analogue of `/effort`: it sets **how strict the pass threshold is** for `/loop-feature` and `/loop-refactor`. The setting lives as `gate_strictness:` in each skill's `resources/project.config.md`, and both loops read it when they evaluate the QA panel.
...
4. **Apply to every config.** For each `project.config.md` found, set the mode: if a `gate_strictness:` line exists, replace its value; if not, add the line (right after `playwright:` if present, else near the top). Keep the trailing explanatory comment. Update **all** of them so `/loop-feature` and `/loop-refactor` agree.
5. **Confirm.** Print the chosen mode, the resulting pass threshold (the table row), and the list of config files updated. Fire `bash ~/.claude/notify-telegram.sh "[<project>] /loop-gate set to <mode>"`. The change takes effect on the **next** `/loop-feature` or `/loop-refactor` run (the loops read `gate_strictness` when they evaluate the panel) — no reload needed.
