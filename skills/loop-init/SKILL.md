...
compatibility: "Needs uv/uvx (for Spec Kit) and network access (web search + the spec-kit clone). Bootstraps the .specify/ structure + .specify/memory/constitution.md that /loop-feature and /loop-refactor depend on. The spec-loop skills + QA agents themselves are installed by install.sh; loop-init verifies them and points back to install.sh if any are missing."
metadata:
  author: "khairul"
  version: "1.0.0"
...
4. **Superpowers (verify + instruct).** `/loop-fix` needs the Superpowers plugin (`obra/Superpowers`) for its `systematic-debugging` + `verification-before-completion` skills. Check whether those skills are available in this session. If **not**, tell the user to install it — `/plugin marketplace add obra/superpowers-marketplace` then `/plugin install superpowers@superpowers-marketplace` (or `/plugin install superpowers@claude-plugins-official`) — and **reload Claude Code**. It's a `/plugin` command, so you can't run it programmatically; just instruct, and note `/loop-fix` is the only loop that requires it.
5. **Playwright (web only, optional).** If `resources/project.config.md` exists and its `surface_default` is `web`/`both` while `playwright: absent`, offer to bootstrap it (`install.sh <project> --with-playwright`). It is only needed for `/loop-feature` web parity; skip for mobile/refactor-only projects.
