...
4. **Playwright (web only, optional).** If `resources/project.config.md` exists and its `surface_default` is `web`/`both` while `playwright: absent`, offer to bootstrap it (`install.sh <project> --with-playwright`). It is only needed for `/loop-feature` web parity; skip for mobile/refactor-only projects.
