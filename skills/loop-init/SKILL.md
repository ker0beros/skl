...
4. **Generate each skill's `project.config.md`.** For every installed skill at `.claude/skills/loop-*/resources/`, **create `project.config.md` if it doesn't already exist** (never overwrite an existing one — it's the user's). Detect the values:
   ...
   Write the file in the shape of `.claude/skills/loop-feature/resources/project.config.example.md`, with `gate_strictness: standard` and only the gate lists for the detected surface. (`/loop-gate` and `/loop-update` rely on this file existing in each skill.) For a mobile surface, also point the user at `resources/mobile-render.md` (render runbook) and `resources/no-overflow-testing.md` (the headless-test convention `make test` must follow), and tell them to fill `device_logical_size` before the first design-driven `/loop-feature` round.
