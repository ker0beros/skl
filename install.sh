...
echo "  2) /go-loop needs design access (DesignSync): sign in to claude.ai or run /design-login."
echo "  3) Run:  /go-loop <design project> — <intent>     or     /refactor-loop [scope]"
[ -d "$TARGET/.specify" ] || echo "  (!) Install Spec Kit first — these skills depend on the speckit-* skills."
