#!/usr/bin/env bash
# go-loop installer — drops the /go-loop skill + 7 QA agents into a spec-kit project and
# stamps that project's surface + gate commands into resources/project.config.md.
#
# Usage:
...
# ---- install skill ---------------------------------------------------------
SKILL_DST="$TARGET/.claude/skills/go-loop"
echo "  installing skill → $SKILL_DST"
copy_file "$SOURCE_DIR/skills/go-loop/SKILL.md" "$SKILL_DST/SKILL.md"
for f in "$SOURCE_DIR"/skills/go-loop/resources/*; do copy_file "$f" "$SKILL_DST/resources/$(basename "$f")"; done

# ---- optional: bootstrap Playwright into the skill -------------------------
...
CONFIG="$SKILL_DST/resources/project.config.md"
if [ -f "$CONFIG" ] && [ "$FORCE" != 1 ]; then
  echo "  KEEP existing config (use --force to regenerate): ${CONFIG#$TARGET/}"
else
  {
    echo "# go-loop — project config (generated by install.sh on $(date '+%Y-%m-%d %H:%M:%S'))"
    echo "#"
    echo "# The go-loop SKILL reads this for the surface default + gate commands. Hand-edit freely."
    echo
    ...
    echo
    echo "## Automated gate commands — Phase B step 10. Every listed command must exit 0."
    echo "mobile_gates:"
  ...
  echo "  wrote ${CONFIG#$TARGET/}"
fi

# ---- summary ---------------------------------------------------------------
echo
echo "✅ go-loop installed into $PROJECT_NAME"
echo "   surface=$SURFACE  playwright=$PLAYWRIGHT"
echo "   gates: $ANALYZE_CMD ; $TEST_CMD"
echo
echo "Next:"
echo "  1) Reload Claude Code in $PROJECT_NAME so /go-loop and the QA agents register."
echo "  2) Ensure design access (DesignSync): sign in to claude.ai or run /design-login."
echo "  3) Run:  /go-loop <claude-design project name or uuid> — <one-line intent>"
[ -d "$TARGET/.specify" ] || echo "  (!) Install Spec Kit first — go-loop depends on the speckit-* skills."
